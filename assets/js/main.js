...
// Rock Paper Scissors
let rps = ["ROCK", "PAPER", "SCISSORS"], rpsResetTimeout;
const result = (x, y) => {
...
const createLayout = (x, y) => {
  ...
  for (let i = 0; i < rps.length; i++) {
    ...
    x === rps[i] ? $(buttona).css("font-weight", "bold").css("color", "black") : y === rps[i] ? $(buttona).css("color", "white") : $(buttona).css("font-weight", "light").css("color", "var(--yellow)");
