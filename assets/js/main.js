...
function oneMinute() {
  ...
  do {
    drowCheck = createRandomWord();
  } while (drowCheck.length <= 19 || drowCheck.length >= 21);
  drow = drowCheck;
  // create random words
  $("#section9")
    .text(drow)
...
const createLayout = (x, y, pw, uw) => {
  ...
  for (let i = 0; i < rps.length; i++) {
    ...
    // x === rps[i] && (z === true) & (w === false) ? $(buttona).css("font-weight", "light").css("color", "black").css("text-decoration", "line-through") : x === rps[i] && z === false && w === true ? $(buttona).css("font-weight", "bold").css("color", "white") : y === rps[i] && z === true && w === true ? $(buttona).css("font-weight", "light").css("color", "black").css("text-decoration", "line-through") : y === rps[i] && z === false && w === false ? $(buttona).css("font-weight", "bold").css("color", "white") : $(buttona).css("font-weight", "light").css("color", "var(--yellow)");
