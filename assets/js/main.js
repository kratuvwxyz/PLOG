...
const result = (x, y) => {
  ...
  (x === "PAPER" && y === "SCISSORS") ||
  (x === "SCISSORS" && y === "ROCK")
    ? $(".sec10").addClass("btn-danger")
    : $(".sec10").addClass("btn-success");
};
const match = (x, y) => {
...
const startRPS = (x) => {
  $(".sec10")
    .removeClass("btn-info btn-danger btn-success")
    .addClass("btn-primary");
  let compRPS = rps[Math.floor(Math.random() * rps.length)];
  createLayout(compRPS, x);
...
const createLayout = (x, y) => {
  ...
  for (let i = 0; i < rps.length; i++) {
    let buttona = $("<button>");
    $(buttona)
      .addClass("btn btn-primary btn-lg sec10")
      .text(rps[i])
      .attr("value", rps[i])
      .attr("onclick", "getrpsvalue(this)");
    x === rps[i]
      ? $(buttona).css("font-weight", "bold").css("color", "black")
      : y === rps[i]
      ? $(buttona).css("color", "white")
      : $(buttona).css("font-weight", "light").css("color", "var(--yellow)");
