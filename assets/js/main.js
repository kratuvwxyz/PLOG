...
const recordPlog = () => {
  let recordTime;
  do {
    recordTime = parseInt(prompt("How long do you want to focus on your task? \n\n(Make it 2 or more minutes)?", 2));
  } while (isNaN(recordTime) || recordTime - 1 <= 0);
  let date = parseInt(recordTime * 60 * 1000);
  const myInterval = setInterval(myTimer, 1000);

  function myTimer() {
    document.getElementById("section14").innerHTML = date;
    date === 0 ? myStopFunction() : (date = date - 1000);
  }
